(function () {
  ...
  // ===========================================================
  // MOTOR DE SIMILARIDADE TEXTUAL (fuzzy matching) — portado do
  // motor de comparação do Autoorganizador (Python/difflib) pra JS.
  // Camada A MAIS: pega títulos que são VARIAÇÕES de um termo já
  // bloqueado manualmente (erro de digitação, abreviação, ordem
  // trocada), mesmo quando o bloqueio exato não bate.
  // ===========================================================
  const MotorSimilaridade = (function () {
    const PALAVRAS_RUIDO = new Set([
      "de", "do", "da", "dos", "das", "no", "na", "nos", "nas",
      "um", "uma", "uns", "umas", "o", "a", "os", "as", "e", "ou", "com",
      "por", "para", "pra", "que", "se", "em", "ao", "aos",
      "video", "videos", "shorts", "short", "canal", "assista", "veja"
    ]);
    ...
    function extrairPalavras(texto) {
      ...
      return norm.match(/[\wà-úÀ-Ú]+/g) || [];
    }

    // difflib.SequenceMatcher(None, a, b).ratio() — Ratcliff/Obershelp
    function maiorBlocoComum(a, b, aLo, aHi, bLo, bHi) {
      let melhorI = aLo, melhorJ = bLo, melhorTam = 0;
      let j2len = {};
      for (let i = aLo; i < aHi; i++) {
        ...
        for (let j = bLo; j < bHi; j++) {
          if (a[i] === b[j]) {
            ...
            novoJ2len[j] = k;
            if (k > melhorTam) { melhorI = i - k + 1; melhorJ = j - k + 1; melhorTam = k; }
          }
        }
        j2len = novoJ2len;
      }
      return { i: melhorI, j: melhorJ, tamanho: melhorTam };
    }

    function razaoSequencia(a, b) {
      if (!a || !b) return 0;
      if (a === b) return 1;
      let somaBlocos = 0;
      const pilha = [[0, a.length, 0, b.length]];
      while (pilha.length) {
        const [aLo, aHi, bLo, bHi] = pilha.pop();
        const bloco = maiorBlocoComum(a, b, aLo, aHi, bLo, bHi);
        if (bloco.tamanho === 0) continue;
        somaBlocos += bloco.tamanho;
        if (aLo < bloco.i && bLo < bloco.j) pilha.push([aLo, bloco.i, bLo, bloco.j]);
        if (bloco.i + bloco.tamanho < aHi && bloco.j + bloco.tamanho < bHi) {
          pilha.push([bloco.i + bloco.tamanho, aHi, bloco.j + bloco.tamanho, bHi]);
        }
      }
      return (2 * somaBlocos) / (a.length + b.length);
    }

    function analisarPadraoTitulo(palavras1, palavras2) {
      const pos1 = palavras1.map((w, i) => [i, w]).filter(([, w]) => w.length > 4 && !PALAVRAS_RUIDO.has(w));
      const pos2 = palavras2.map((w, i) => [i, w]).filter(([, w]) => w.length > 4 && !PALAVRAS_RUIDO.has(w));
      let acertos = 0, comparacoes = 0;
      for (const [p1, w1] of pos1) {
        for (const [p2, w2] of pos2) {
          if (p1 === p2) {
            comparacoes++;
            if (w1 === w2 || razaoSequencia(w1, w2) > 0.8) acertos++;
          }
        }
      }
      if (comparacoes === 0) return 0.5;
      return acertos / comparacoes;
    }

    function analisarPalavrasExtras(palavras1, palavras2) {
      const set1 = new Set(palavras1);
      const set2 = new Set(palavras2);
      const comuns = [...set1].filter((w) => set2.has(w));
      const so1 = [...set1].filter((w) => !set2.has(w));
      const so2 = [...set2].filter((w) => !set1.has(w));
      let penalidade = 0;
      for (const w of [...so1, ...so2]) if (w.length > 3 && !PALAVRAS_RUIDO.has(w)) penalidade += 0.3;
      if (!set1.size || !set2.size) return 0;
      const base = comuns.length / Math.max(set1.size, set2.size);
      return Math.max(0, base - Math.min(penalidade, 0.7));
    }

    function analisarOrdemPalavras(palavras1, palavras2) {
      let preservada = 0, comparacoes = 0;
      for (let i = 0; i < palavras1.length; i++) {
        const w = palavras1[i];
        const pos2 = palavras2.indexOf(w);
        if (pos2 === -1) continue;
        if (i > 0 && palavras2.includes(palavras1[i - 1])) {
          const posAnt2 = palavras2.indexOf(palavras1[i - 1]);
          if (pos2 > posAnt2) preservada++;
          comparacoes++;
        }
        if (i < palavras1.length - 1 && palavras2.includes(palavras1[i + 1])) {
          const posProx2 = palavras2.indexOf(palavras1[i + 1]);
          if (pos2 < posProx2) preservada++;
          comparacoes++;
        }
      }
      if (comparacoes === 0) return razaoSequencia(palavras1.join(" "), palavras2.join(" "));
      return preservada / comparacoes;
    }

    function analisarPosicoesCriticas(palavras1, palavras2) {
      const n = Math.min(3, palavras1.length, palavras2.length);
      if (n === 0) return 0;
      let acertos = 0;
      for (let i = 0; i < n; i++) {
        if (palavras1[i] === palavras2[i]) acertos += 1;
        else if (razaoSequencia(palavras1[i], palavras2[i]) > 0.85) acertos += 0.5;
      }
      return acertos / n;
    }

    // Score combinado 0-1 entre um texto (título/canal do vídeo) e um
    // termo bloqueado — mesmos pesos do motor original em Python
    // (0.35 padrão / 0.25 extras / 0.25 ordem / 0.15 posição)
    function compararComTermo(texto, termoBloqueado) {
      const palavrasTexto = extrairPalavras(texto);
      const palavrasTermo = extrairPalavras(termoBloqueado);
      if (!palavrasTexto.length || !palavrasTermo.length) return 0;
      const scorePadrao = analisarPadraoTitulo(palavrasTexto, palavrasTermo);
      const scoreExtras = analisarPalavrasExtras(palavrasTexto, palavrasTermo);
      const scoreOrdem = analisarOrdemPalavras(palavrasTexto, palavrasTermo);
      const scorePosicao = analisarPosicoesCriticas(palavrasTexto, palavrasTermo);
      return scorePadrao * 0.35 + scoreExtras * 0.25 + scoreOrdem * 0.25 + scorePosicao * 0.15;
    }

    function melhorCorrespondencia(texto, listaTermos) {
      let melhorScore = 0, melhorTermo = null;
      for (const termo of listaTermos) {
        const score = compararComTermo(texto, termo);
        if (score > melhorScore) { melhorScore = score; melhorTermo = termo; }
      }
      return { score: melhorScore, termo: melhorTermo };
    }

    return { extrairPalavras, razaoSequencia, compararComTermo, melhorCorrespondencia };
  })();

  // Limiar de corte pro fuzzy — ajuste se estiver bloqueando demais (subir)
  // ou de menos (descer). 0.72 é um ponto de partida conservador.
  const LIMIAR_FUZZY = 0.72;

  function bloqueadoPorFuzzy(titulo) {
    if (!titulo) return null;
    const termosParaComparar = GM_getValue(STORAGE_KEY_TITLES, []).map(stripQuotes);
    if (!termosParaComparar.length) return null;
    const resultado = MotorSimilaridade.melhorCorrespondencia(titulo, termosParaComparar);
    return resultado.score >= LIMIAR_FUZZY ? resultado : null;
  }
      ...
      const html = `
          <div id="yt-blocker-settings">
              <div id="yt-blocker-settings-content">
                  ...
                  <div class="yt-blocker-section-desc">Uma linha por conjunto (ex: vlog diario). Bloqueia se TODAS as palavras da linha estiverem no título. Isso é além do filtro por pontuação e do filtro difuso, que já rodam automaticamente.</div>
    ...
    if (path === "/watch") {
      ...
      if (idAtual !== ultimoWatchAvaliado) {
        ...
        if (titulo) {
          ...
          const fuzzy = bloqueadoPorFuzzy(titulo);
          if (fuzzy) {
            if (MODO_DIAGNOSTICO) console.debug("[Filtro][Watch][Fuzzy] score " + fuzzy.score.toFixed(2) + " vs \"" + fuzzy.termo + "\":", titulo);
            paginaJaBloqueada = true;
            window.location.replace(window.location.hostname.startsWith("m.") ? "https://m.youtube.com/" : "https://www.youtube.com/");
            return;
          }
  ...
  function checarShortAtual() {
    ...
    // 3) camada difusa (fuzzy) contra os termos já bloqueados
    let resultadoFuzzy = null;
    if (!bloqueado) {
      resultadoFuzzy = bloqueadoPorFuzzy(titulo);
      if (resultadoFuzzy) bloqueado = true;
    }

    // 4) camada heurística por pontuação
    ...
    if (MODO_DIAGNOSTICO && resultadoFuzzy) {
      console.debug("[Filtro][Short][Fuzzy] score " + resultadoFuzzy.score.toFixed(2) + " vs \"" + resultadoFuzzy.termo + "\":", titulo);
    }
  ...
  function blockContent(force = false) {
    ...
    cards.forEach((card) => {
      ...
      // 3) camada difusa (fuzzy) — variações de termos já bloqueados
      let resultadoFuzzy = null;
      if (!shouldBlock) {
        resultadoFuzzy = bloqueadoPorFuzzy(findVideoTitle(card));
        if (resultadoFuzzy) shouldBlock = true;
      }
      ...
      if (MODO_DIAGNOSTICO && resultadoFuzzy) {
        console.debug("[Filtro][Card][Fuzzy] score " + resultadoFuzzy.score.toFixed(2) + " vs \"" + resultadoFuzzy.termo + "\":", findVideoTitle(card));
      }
