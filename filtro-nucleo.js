...
(function () {
  ...
  function buscarConteudoGitHub(url) {
    return new Promise((resolve, reject) => {
      if (!url || url.startsWith("COLE_AQUI")) { reject(new Error("URL nao configurada")); return; }
      // "cache: no-store" via parâmetro na URL evita pegar versão
      // antiga em cache do CDN do GitHub
      ...
      GM_xmlhttpRequest({
        ...
        onload: (resp) => {
          try {
            ...
            const linhas = (resp.responseText || "")
              .split("\n")
              .map((l) => l.trim())
              .filter(Boolean);
            resolve(linhas);
  ...
  GM_registerMenuCommand("Bloquear este canal", blockCurrentChannel);
  GM_registerMenuCommand("Bloquear este vídeo", blockCurrentVideo);
  GM_registerMenuCommand("Atualizar filtros agora (GitHub)", () => atualizarDoGitHub(true));
